...
def monte_carlo_3d(t, omega_H, omega_L, T, N_ensemble=1000,
...
def fit_gs_tweezer_recapture(recapture_rate, t):
    from lmfit import Model

    model = Model(recapture_rate_3d, independent_vars=['t'])
    params = model.make_params(omega_H=0.8e6, omega_L=0.18e6,
                               trap_temp=2.63e-3,
                               T_ensemble=20e-6, mass=Caesium().mass)
    params['mass'].vary = False
    params['omega_H'].vary = False
    params['omega_L'].vary = False

    result = model.fit(recapture_rate, params, t=t)
...
def run_fit():
    ...
    best_fit = fit_gs_tweezer_recapture(recapture, time)
