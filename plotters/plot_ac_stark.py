...
def plot_Cs_7p_tweezer_shift():
	shifter = ac_stark.ACStarkShift(n=7, l=1, j=1.5)
	wavelengths = np.linspace(440, 1080, 2000) * 1e-9
	powers = np.asarray([20e-3])
	shirley_shift_wavelength_7P32 = shifter.ac_stark_shift_shirley(wavelengths,
																 powers).squeeze()
	
	powers = np.linspace(0, 20e-3, 1000)
	shirley_shift_power_7P32 = shifter.ac_stark_shift_shirley(np.asarray([1070])
															* 1e-9,
															powers).squeeze()
	
	shifter_12 = ac_stark.ACStarkShift(n=7, l=1, j=0.5)
	powers = np.asarray([20e-3])
	shirley_shift_wavelength_7P12 = shifter_12.ac_stark_shift_shirley(
		wavelengths,
																 powers).squeeze()
	
	powers = np.linspace(0, 20e-3, 1000)
	shirley_shift_power_7P12 = shifter_12.ac_stark_shift_shirley(np.asarray([
		1070])
															* 1e-9,
															powers).squeeze()
	...
	ax1.plot(wavelengths * 1e9, shirley_shift_wavelength_7P12 * 1e-6,
			 '--',
			 label=f"state = {shifter_12.state}")
	ax1.set_xlabel("wavelength (nm)")
